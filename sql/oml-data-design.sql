...
INSERT INTO profile VALUES (HEX('1'), null, 'ariljohnson@gmail.com', 'abc123', 'ArinJ', null, HEX(123456789)),
                           (HEX('2'), null, 'feliciakeys@gmail.com', 'dogsRcool', 'PuppyFelicia', null, HEX(223456789)),
                           (HEX('3'), null, 'warren14532@yahoo.com', 'bananas!', 'BananaRama', null, HEX(323456789));

-- Write and execute write an update statement on one of the rows created in the previous step,
UPDATE profile SET profileDisplayName = 'ArinDaBoss' WHERE profileEmail = 'ariljohnson@gmail.com';

-- Write and execute a delete statement on another row created from your insert statements.
DELETE FROM profile WHERE profileEmail = 'feliciakeys@gmail.com';

-- Write and execute an insert statement on a table with a foreign key from the original table.
INSERT INTO gameInformation VALUES (HEX('a'), (SELECT profileId FROM profile WHERE profileEmail='ariljohnson@gmail.com'),
                                    'Minecraft', NULL, 'Live');

-- Write and execute a select statement for a row using its primary key as the selector. ERROR
SELECT * FROM profile WHERE profileId = '31';

-- Write and execute a select statement that incorporates both a where clause and an inner-join on both tables
--      used in this assignment.
SELECT * FROM profile WHERE profileId = '31' INNER JOIN profile.profileId = gameInformation.gameHost;
